...
@implementation GGBaseIPNRSAUtil
/*
...
+ (SecKeyRef)addPublicKey:(NSString *)key{
    ...
    data = [GGBaseIPNRSAUtil stripPublicKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (SecKeyRef)addPrivateKey:(NSString *)key{
    ...
    NSData *data = base64_decode(key);
    data = [GGBaseIPNRSAUtil stripPrivateKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (NSString *)encryptString:(NSString *)str privateKey:(NSString *)privKey{
    NSData *data = [GGBaseIPNRSAUtil encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] privateKey:privKey];
...
+ (NSData *)encryptData:(NSData *)data privateKey:(NSString *)privKey{
    ...
    SecKeyRef keyRef = [GGBaseIPNRSAUtil addPrivateKey:privKey];
    if(!keyRef){
        return nil;
    }
    return [GGBaseIPNRSAUtil encryptData:data withKeyRef:keyRef];
}
...
+ (NSString *)decryptString:(NSString *)str privateKey:(NSString *)privKey{
    ...
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [GGBaseIPNRSAUtil decryptData:data privateKey:privKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
...
+ (NSData *)decryptData:(NSData *)data privateKey:(NSString *)privKey{
    ...
    return [GGBaseIPNRSAUtil decryptData:data withKeyRef:keyRef];
}
...
+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey{
    ...
    NSData *data = [GGBaseIPNRSAUtil encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKey:pubKey];
    NSString *ret = base64_encode_data(data);
    return ret;
}
...
+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
    ...
    SecKeyRef keyRef = [GGBaseIPNRSAUtil addPublicKey:pubKey];
    if(!keyRef){
        return nil;
    }
    return [GGBaseIPNRSAUtil encryptData:data withKeyRef:keyRef];
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [GGBaseIPNRSAUtil decryptData:data publicKey:pubKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
...
+ (BOOL)rsaVerifySignature:(NSString *)signature plainString:(NSString *)plainString WithPublicKey:(NSString *)publickey{
    ...
    SecKeyRef key = [GGBaseIPNRSAUtil addPublicKey:publickey];
...
+ (NSString *)rsaSignString:(NSString *)plainString WithPrivateKey:(NSString *)privateKey{
    ...
    SecKeyRef key = [GGBaseIPNRSAUtil addPrivateKey:privateKey];
    size_t signedHashBytesSize = SecKeyGetBlockSize(key);
...
+ (NSString *) sign:(NSString *)storString privateKey:(NSString *) privateKey{
    // 使用哈希算法获取字符串摘要
    // sha256加密
    NSData *outData = [self sha256:storString];
    SecKeyRef pKey = [GGBaseIPNRSAUtil addPrivateKey:privateKey];
    if (!pKey) {
        return @"";
    }
